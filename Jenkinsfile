...
pipeline {
  ...
  stages {
    ...
    stage('build') {
      steps {
        ...
        // npm ci is used therefor you need to have the package-lock checked in (which is generated once you run npm install)
      ...
      parallel {
        ...
        stage('regression test') {
          steps {
            ...
            sh "export NO_COLOR=1 && npm run cy:dev:run:ci"
          }
        }
  ...
  post {
    always {
      echo 'Archieving reports'
      archiveArtifacts artifacts: 'cypress/screenshots/**, cypress/videos/**', followSymlinks: false
      junit 'results/**'
      cleanWs()
    }
  }
